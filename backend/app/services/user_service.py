...
class UserService:
    ...
    async def create(self, org_id: int, payload: UserCreate, actor_user_id: int) -> tuple[User, str]:
        ...
        for role_id in payload.role_ids:
            self.repo.add_role(UserRole(user_id=user.id, role_id=role_id, at_organization_id=org_id))
        token = new_verification_token()
        _user_verification_tokens[token] = (org_id, user.id)
        await self.audit.write(org_id, "USER_CREATED", "USER", f"{user.email} created", actor_user_id=actor_user_id, target_user_id=user.id)
        await self.session.commit()
        return user, token

    async def update(self, org_id: int, user_id: int, payload: UserUpdate) -> User:
    ...
    async def delete(self, org_id: int, user_id: int) -> None:
        user = await self.repo.get_scoped(org_id, user_id)
        if not user:
            raise AppError(404, "USER_NOT_FOUND", "User was not found")
