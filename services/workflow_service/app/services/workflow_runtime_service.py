...
class WorkflowRuntimeService:
    ...
    async def start_workflow(
        ...
        assignees = await self._assignee_svc.resolve_assignees(
            db, next_node, request.payload, auth_header, current_user["user_id"]
        )
    ...
    async def get_instance_detail(
        ...
        workflow = await db.get(WorkflowDefinition, instance.workflow_id)
        ...
        node_rows = (
            await db.execute(
                select(WorkflowNode).where(
                    ...
                    WorkflowNode.is_deleted == False,
                ).order_by(WorkflowNode.sequence_no.asc().nullslast(), WorkflowNode.node_id.asc())
            )
        ).scalars().all()
        ...
        action_rows = (
            await db.execute(
                select(WorkflowAction).where(
                    WorkflowAction.node_id.in_([n.node_id for n in node_rows]),
                    WorkflowAction.is_deleted == False,
                    WorkflowAction.is_active == True,
                ).order_by(WorkflowAction.node_id.asc(), WorkflowAction.display_order.asc())
            )
        ).scalars().all()
        actions_by_node: dict[int, list[WorkflowAction]] = {}
        ...
        task_rows = (
            await db.execute(
                select(WorkflowTask).where(
                    ...
                    WorkflowTask.is_deleted == False,
                ).order_by(WorkflowTask.created_at.asc(), WorkflowTask.workflow_task_id.asc())
            )
        ).scalars().all()
        ...
        return {
            ...
            "workflow": {
                ...
                "trigger_action": workflow.trigger_action,
                "entity_name": workflow.entity_name,
                "entity_table_name": workflow.entity_table_name,
            } if workflow else None,
            "entity_name": instance.entity_name,
            "entity_table_name": instance.entity_table_name,
            "entity_record_id": instance.entity_record_id,
            "workflow_status": instance.workflow_status,
            ...
            "steps": [
                {
                    ...
                    "step_code": node.node_code,
                    "step_name": node.node_name,
                    "node_type": node.node_type,
                    "sequence_no": node.sequence_no,
                    "is_start_node": node.is_start_node,
                    "is_end_node": node.is_end_node,
                    "assignee_type": node.assignment_type,
                    "assigned_user_id": node.assigned_user_id,
                    "assigned_role_id": node.assigned_role_id,
                    "assignee_role_code": (
                        "ORG_ADMIN"
                        if workflow
                        and workflow.workflow_code == "TASK_APPROVAL_WORKFLOW"
                        and node.assigned_role_id == settings.DEFAULT_ORG_ADMIN_ROLE_ID
                        else None
                    ),
                    ...
                    "allowed_actions": [
                        {
                            ...
                            "requires_comment": action.requires_comment,
                            "requires_attachment": action.requires_attachment,
                            "is_positive_action": action.is_positive_action,
                            "is_negative_action": action.is_negative_action,
                            "display_order": action.display_order,
                        }
                        for action in actions_by_node.get(node.node_id, [])
                    ],
                }
            ...
            "history": [
                {
                    "workflow_action_history_id": h.workflow_action_history_id,
                    "workflow_task_id": h.workflow_task_id,
            ...
            "tasks": [
                {
                    ...
                    "action_taken_by": task.action_taken_by,
                    "action_taken_at": task.action_taken_at.isoformat()
                    if task.action_taken_at
                    else None,
